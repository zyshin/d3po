...
def main(_):
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        if samples.get("eval_rewards") is not None:
            ...
            accelerator.log(
                {"reward": eval_rewards, "num_samples": epoch*available_devices*config.sample.batch_size, "reward_mean": eval_rewards.mean(), "reward_std": eval_rewards.std()},
                step=global_step,
            )
        else:
            accelerator.log(
            ...
            for i, sample in tqdm(
                ...
                for j in tqdm(
                    ...
                    with accelerator.accumulate(pipeline.unet):
                        ...
                        kl_divergence = kl.kl_divergence(torch.distributions.Categorical(logits=log_prob), torch.distributions.Categorical(logits=sample["log_probs"][:, j]))
